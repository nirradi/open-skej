...
describe('SpaceSchedulePanel', () => {
  it('shows the Space timezone, hours, and slot interval', () => {
    renderPanel(
      makeSpace({
  ...
  it('saves edited timezone, hours, and slot interval together', async () => {
  ...
  it('clears hours and slot interval back to "no restriction" with an explicit null', async () => {
    const updated = makeSpace({ opens_at: null, closes_at: null, slot_minutes: null })
    vi.mocked(updateSpace).mockResolvedValue(ok(updated))
    renderPanel(makeSpace({ opens_at: '07:00:00', closes_at: '22:00:00', slot_minutes: 60 }))
